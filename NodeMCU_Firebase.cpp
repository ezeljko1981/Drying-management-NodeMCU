/*
  NodeMCU_Firebase.h - Library for reading values and sending them to Firebase. Also getting param values from Firebase.
...
#define FIREBASE_AUTH "****************************************"    // the secret key generated from firebase Service accounts>Database secrets

#define WIFI_SSID     "********"                                    // put your home or public wifi name
#define WIFI_PASSWORD "********"                                    // put your password of wifi ssid
...
//time offset
const long utcOffsetInSeconds = 3600;  //UTC offset for your timezone in milliseconds
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "europe.pool.ntp.org", utcOffsetInSeconds);
char daysOfTheWeek[7][12] = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};

// sensors and actuators
Dht22   dht22_un;  //Object of Dht22 sensor inner
Dht22sp  dht22_sp;  //Object of Dht22 sensor outer
int relayP;
int fanP;
Relay relayH;    // Heater
Relay relayF;    // Fan(s)
...
void NodeMCU_Firebase::Loop()
{
  ...
  switch (m_State)
  {
    ...
    case MEASURE_STATE:
      ...
      m_h_un = dht22_un.ReadDht22humidity();
      m_t_sp = dht22_sp.ReadDht22temperature();
      m_h_sp = dht22_sp.ReadDht22humidity();
      delay(3000);
      GetParamsFromDB();
    ...
    case CALCULATE_STATE:
      //Serial.println("CALCULATE_STATE");
      BusinessLogic();
    ...
    case WRITING_STATE:
      ...
      //node: current
      delay(250);
      Firebase.setFloat("/drier_temp_keeping/current/t_un", m_t_un);
      Firebase.setFloat("/drier_temp_keeping/current/h_un", m_h_un);
      Firebase.setFloat("/drier_temp_keeping/current/t_sp", m_t_sp);
      Firebase.setFloat("/drier_temp_keeping/current/h_sp", m_h_sp);
      delay(250);
      Firebase.setFloat("/drier_temp_keeping/current/dayofweek", m_dayOfWeek);
      Firebase.setString("/drier_temp_keeping/current/time", m_time);
      Firebase.setInt("/drier_temp_keeping/current/limit_r", temp_target + temp_margin_top);
      Firebase.setInt("/drier_temp_keeping/current/limit_l", temp_target - temp_margin_bottom);
      //node: archive
      Firebase.setFloat("/drier_temp_keeping/" + m_archive_name + "/" + smc + "/t_un", m_t_un);
      Firebase.setFloat("/drier_temp_keeping/" + m_archive_name + "/" + smc + "/h_un", m_h_un);
      Firebase.setFloat("/drier_temp_keeping/" + m_archive_name + "/" + smc + "/t_sp", m_t_sp);
      Firebase.setFloat("/drier_temp_keeping/" + m_archive_name + "/" + smc + "/h_sp", m_h_sp);
...
void NodeMCU_Firebase::BusinessLogic() {
  if (m_t_un > (temp_target + temp_margin_top)) { //iznad smo
    HeaterOff();
  } else if (m_t_un < (temp_target - temp_margin_bottom)) { //ispod smo
    HeaterOn();
  }
}
